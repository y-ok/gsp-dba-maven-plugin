...
public class ExecuteDdlMojoTest extends AbstractDdlMojoTest<ExecuteDdlMojo> {
    ...
    public void testExecSqlFile() throws Exception {
        ...
        for (MojoTestFixture mf : mojoTestFixtureList) {
            ...
            // スキーマのドロップ
            DBTestUtil.dropSchema(mojo.schema, mojo.user, mojo.password, mojo.adminUser,
                    mojo.adminPassword, mojo.url, mojo.driver, mf.testDb);
    ...
    public void testBasic() throws Exception {
        ...
        for (MojoTestFixture mf : mojoTestFixtureList) {
            ...
            if (!(mf.testDb.equals(TestDB.db2))) {
                DBTestUtil.dropUser(mojo.schema, mojo.user, mojo.password, mojo.adminUser,
                        mojo.adminPassword, mojo.url, mojo.driver, mf.testDb);
            }
     ...
     */
    @Test
    @TestDBPattern(testCase = "exist_schema",
            testDb = {TestDB.oracle, TestDB.postgresql, TestDB.db2, TestDB.h2, TestDB.sqlserver})
     ...
     */
    @Test
    @TestDBPattern(testCase = "exist_another_schema",
     ...
     */
    @Test
    @TestDBPattern(testCase = "extraDdlDirectory", testDb = {TestDB.oracle, TestDB.postgresql,
     ...
     */
    @Test
    @TestDBPattern(testCase = "another_schema_crud",
            testDb = {TestDB.oracle, TestDB.postgresql, TestDB.db2, TestDB.sqlserver})
    public void testAnotherSchemaCrud() throws Exception {
        ...
        for (MojoTestFixture mf : mojoTestFixtureList) {
            ...
            try {
                ...
                String insertSql = "INSERT INTO " + mojo.schema
                        + ".TEST_TBL1(TEST_NAME) VALUES('TEST_INSERT')";
                String updateSql =
                        "UPDATE " + mojo.schema + ".TEST_TBL1 SET TEST_NAME = 'TEST_UPDATE'";
                ...
                switch (mf.testDb) {
                    ...
                    case oracle:
                        insertSql = "INSERT INTO " + mojo.schema
                                + ".TEST_TBL1(TEST_TBL1_ID, TEST_NAME) VALUES(" + mojo.schema
                                + ".TEST_TBL1_ID_SEQ.nextval, 'TEST_INSERT')";
                        break;
                    case db2:
                        break;
                    case h2:
     ...
     * <li>optionalDialectsによりDialectクラスを差し替えることが出来るかをテスト。</li>
     * <li>h2だけでテスト。</li>
     * <li>gspに標準装備されていないjdbcURLのＤＢプロトコルで、optionalDialectsを差し込むことが出来ることをテスト。</li>
     * <li>このテストで差し込むoptionalDialectsはオブジェクト生成時に例外を投げるようにし、これを期待値とする。</li>
     * </ul>
     * <h4>検証結果</h4>
     * <ul>
