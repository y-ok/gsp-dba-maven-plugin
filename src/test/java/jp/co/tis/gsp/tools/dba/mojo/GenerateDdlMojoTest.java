...
public class GenerateDdlMojoTest extends AbstractDdlMojoTest<GenerateDdlMojo> {
     ...
     */
    @Test
    @TestDBPattern(testCase = "ddlTemplateFileDir", testDb = {TestDB.oracle, TestDB.postgresql,
    ...
    @TestDBPattern(testCase = "another_schema",
            testDb = {TestDB.oracle, TestDB.postgresql, TestDB.sqlserver, TestDB.db2})
     ...
     */
    @Test
    @TestDBPattern(testCase = "auto_increment",
            testDb = {TestDB.oracle, TestDB.db2, TestDB.sqlserver, TestDB.mysql})
    ...
    public void testSpecifiedSchemaInEdm() throws Exception {
        for (MojoTestFixture mojoTestFixture : mojoTestFixtureList) {
            ...
            GenerateDdlMojo mojo =
                    this.lookupConfiguredMojo(file, GENERATE_DDL, mojoTestFixture.testDb);
            mojo.execute();

            // 検証
            final String actualPath = mojo.outputDirectory.getAbsolutePath();
            final Entry actualFiles = DirUtil.collectEntry(actualPath);
            final Entry expectedFiles =
                    DirUtil.collectEntry(getExpectedPath(mojoTestFixture) + FS + "ddl");
            assertThat("TestDb:" + mojoTestFixture.testDb, actualFiles.equals(expectedFiles),
                    is(true));
        }
    }
}
